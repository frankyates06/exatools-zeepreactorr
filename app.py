...
def switch_page(url, pure_url):
  ...
  progress_bar = st.progress(0)  # Initialize progress bar outside the loop
  while count <= limite:
      dl_intel(url, pure_url)
      progress_text = f'Progression: {np.round((count/limite)*100)}%'
      st.text(progress_text)  # Dynamic text display in Streamlit
      ...
      for lines in K.readlines():
          Results.write(lines)
      K.close()
      progress_bar.progress(count/limite)  # Update progress bar
  Results.close()
  progress_bar.empty()  # Clear progress bar after loop
def sci(keywords):    
  ...
  for i in F.readlines(): 
      i = i.strip('\n').split('\t')
      progress_text = f'article n° {str(signal)}\tloading : {np.round((signal/limite)*100)}%'
      st.text(progress_text)  # Dynamic text display in Streamlit
      signal += 1
      ...
      progress_bar.progress(signal/limite)  # Update progress bar
  ...
  if b'%PDF' in raw_data:
      ...
      try:
          ...
          for page in range(len(read_pdf.pages)):
              ...
              if txt:  # Check if txt is not None
